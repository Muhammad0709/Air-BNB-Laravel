      ...
      <AdminLayout title="Create Support Ticket">
        ...
        <Card elevation={0} sx={{ border: '1px solid #E5E7EB', borderRadius: 2 }}>
          <CardContent sx={{ p: { xs: 2, md: 4 } }}>
            <Typography variant="h5" sx={{ fontWeight: 700, color: '#222222', mb: 4 }}>Ticket Information</Typography>
            <form onSubmit={handleSubmit}>
              <Row>
                <Col xs={12} md={6}>
                  <Stack spacing={3} sx={{ mb: { xs: 3, md: 0 } }}>
                    <TextField label="Subject" name="subject" value={formData.subject} onChange={handleChange} required fullWidth placeholder="e.g., Payment issue with booking" />
                    <TextField label="User Name" name="user" value={formData.user} onChange={handleChange} required fullWidth placeholder="e.g., John Doe" />
                    <TextField label="Email" name="email" type="email" value={formData.email} onChange={handleChange} required fullWidth placeholder="e.g., john@example.com" />
                  </Stack>
                </Col>
                <Col xs={12} md={6}>
                  <Stack spacing={3}>
              ...
              <Row className="mt-3"><Col xs={12}><TextField label="Description" name="description" value={formData.description} onChange={handleChange} required fullWidth multiline rows={6} placeholder="Describe the issue in detail..." /></Col></Row>
              <Row className="mt-4">
                <Col xs={12}>
                  <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} justifyContent="flex-end" sx={{ width: '100%' }}>
                    ...
                    <Button type="submit" variant="contained" sx={{ bgcolor: '#AD542D', textTransform: 'none', fontWeight: 700, '&:hover': { bgcolor: '#78381C' } }}>Create Ticket</Button>
                  </Stack>
                </Col>
              </Row>
            </form>
        ...
        <Toast open={toastOpen} onClose={() => setToastOpen(false)} message="Support ticket created successfully!" severity="success" />
      </AdminLayout>
    </>
  )
}
