  ...
  return (
    <>
      ...
      <AdminLayout title={t('admin.support_tickets.create')}>
        ...
        <Card elevation={0} sx={{ border: '1px solid #E5E7EB', borderRadius: 2 }}>
          <CardContent sx={{ p: { xs: 2, md: 4 } }}>
            ...
            <form onSubmit={handleSubmit}>
              <Row>
                <Col xs={12} md={6}>
                  <Stack spacing={3} sx={{ mb: { xs: 3, md: 0 } }}>
                    <TextField label={t('admin.support_tickets.subject')} name="subject" value={formData.subject} onChange={handleChange} required fullWidth placeholder={t('admin.support_tickets.subject_placeholder')} />
                    <TextField label={t('admin.support_tickets.user_name')} name="user" value={formData.user} onChange={handleChange} required fullWidth placeholder={t('admin.support_tickets.user_placeholder')} />
                    <TextField label={t('admin.support_tickets.email')} name="email" type="email" value={formData.email} onChange={handleChange} required fullWidth placeholder={t('admin.support_tickets.email_placeholder')} />
                  </Stack>
                </Col>
                <Col xs={12} md={6}>
                  <Stack spacing={3}>
              ...
              <Row className="mt-3"><Col xs={12}><TextField label={t('admin.support_tickets.description')} name="description" value={formData.description} onChange={handleChange} required fullWidth multiline rows={6} placeholder={t('admin.support_tickets.description_placeholder')} /></Col></Row>
              <Row className="mt-4">
                <Col xs={12}>
                  <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} useFlexGap justifyContent="flex-end" sx={{ width: '100%' }}>
                    ...
                    <Button type="submit" variant="contained" sx={{ bgcolor: '#AD542D', textTransform: 'none', fontWeight: 700, '&:hover': { bgcolor: '#78381C' } }}>{t('admin.support_tickets.create_ticket')}</Button>
                  </Stack>
                </Col>
              </Row>
            </form>
        ...
        <Toast open={toastOpen} onClose={() => setToastOpen(false)} message={t('admin.support_tickets.created_success')} severity="success" />
      </AdminLayout>
    </>
  )
}
