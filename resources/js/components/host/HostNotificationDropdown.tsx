...
export default function HostNotificationDropdown() {
    ...
    const fetchNotifications = async () => {
        try {
            const response = await fetch('/host/notifications/latest', {
                headers: {
    ...
    const fetchUnreadCount = async () => {
        try {
            const response = await fetch('/host/notifications/unread-count', {
                headers: {
                    'Accept': 'application/json',
                    'X-Requested-With': 'XMLHttpRequest',
                },
    ...
    const markAllAsRead = async () => {
        try {
            router.patch('/host/notifications/mark-all-as-read', {}, {
                preserveState: true,
                preserveScroll: true,
                onSuccess: () => {
                    setNotifications(prev =>
    ...
    useEffect(() => {
        ...
        const channel = (window as any).Echo?.private('host-notifications');
        if (channel) {
        ...
        <>
            ...
            >
                        ...
                        notifications.map((notification) => (
                            <MenuItem
                                ...
                                sx={{
                                    py: 2,
                                    px: 2,
                                    bgcolor: notification.read_at ? 'transparent' : '#F3F4F6',
                ...
                <Box sx={{ p: 1.5, textAlign: 'center' }}>
                    <Typography
                        onClick={() => {
                            router.visit('/host/notifications');
                            setAnchorEl(null);
                        }}
                        sx={{
                            fontSize: '0.875rem',
