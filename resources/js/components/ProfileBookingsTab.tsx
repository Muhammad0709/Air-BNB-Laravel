...
export default function ProfileBookingsTab() {
  ...
  const upcomingBookings = allBookings.filter(booking => booking.status === 'Pending')
  const pastBookings = allBookings.filter(booking => booking.status === 'Confirmed' || booking.status === 'Completed')
    ...
    <Box>
      <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 3 }}>
        <Tabs
          ...
          sx={{
            '& .MuiTab-root': { textTransform: 'none', fontWeight: 600, fontSize: '1rem', color: '#717171', minHeight: 48, '&.Mui-selected': { color: '#AD542D' } },
