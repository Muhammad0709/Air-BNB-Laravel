<?php

return [
    'destination' => 'مقصد',
    'checkin' => 'چیک ان',
    'checkout' => 'چیک آؤٹ',
    'guests' => 'مہمان',
    'search' => 'تلاش',
    'adults' => 'بالغ',
    'adult' => 'بالغ',
    'children' => 'بچے',
    'child' => 'بچہ',
    'rooms' => 'کمرے',
    'room' => 'کمرہ',
    'done' => 'ہو گیا',
    'where_to' => 'کہاں؟',
    'when' => 'کب',
    'who' => 'کون',
    'add_dates' => 'تاریخیں شامل کریں',
    'popular_destinations_label' => 'مقبول مقامات',
    'popular_destinations' => 'مقبول مقامات',
    'hotels' => 'ہوٹل',
    'popular' => 'مقبول',
    'favorites' => 'پسندیدہ',
    'no_hotels' => 'کوئی ہوٹل دستیاب نہیں',
    'no_hotels_sub' => 'بعد میں دیکھیں یا دوسری تلاش کریں۔',
    'no_popular' => 'ابھی کوئی مقبول قیام نہیں',
    'no_popular_sub' => 'جیسے جیسے مزید جائیدادیں شامل ہوں گی یہاں دکھائی دیں گی۔',
    'no_favorites' => 'ابھی کوئی پسندیدہ نہیں',
    'no_favorites_sub' => 'کسی لسٹنگ پر دل کے آئیکون پر کلک کر کے جائیدادیں محفوظ کریں۔',
    'search_destinations_placeholder' => 'مقامات تلاش کریں',
    'search_no_suggestions' => 'کوئی مماثل تجاویز نہیں۔ اپنے متن کو مقام کے طور پر استعمال کرنے کے لیے تلاش دبائیں۔',
    'add_guests_placeholder' => 'مہمان شامل کریں',
    'guest_favorite' => 'مہمان کی پسند',
];
