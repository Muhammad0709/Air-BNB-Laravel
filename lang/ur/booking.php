<?php

return [
    'title' => 'اپنی بکنگ مکمل کریں',
    'payment_missing_session' => 'ہمیں آپ کا پیمنٹ سیشن نہیں ملا۔ براہ کرم دوبارہ کوشش کریں۔',
    'payment_not_completed' => 'آپ کی ادائیگی مکمل نہیں ہوئی۔ براہ کرم دوبارہ کوشش کریں۔',
    'payment_dates_unavailable' => 'آپ کی ادائیگی کامیاب رہی، لیکن یہ تاریخیں ابھی کسی اور نے بک کر لی ہیں۔ آپ کو خودکار طور پر رقم واپس کر دی گئی ہے۔',
    'payment_cancelled' => 'ادائیگی منسوخ کر دی گئی۔ آپ سے کوئی رقم نہیں لی گئی۔',
    'your_booking' => 'آپ کی بکنگ',
    'total' => 'کل',
    'guest_details' => 'مہمان کی تفصیلات',
    'name' => 'نام',
    'email' => 'ای میل',
    'phone' => 'فون',
    'payment' => 'ادائیگی',
    'reserve' => 'بک کریں',
    'processing' => 'پروسیس ہو رہا ہے...',
    'select_property' => 'جائیداد منتخب کریں',
    'house_rules' => 'گھر کے اصول',
    'cancellation_policy' => 'منسوخی کی پالیسی',
    'security_deposit' => 'سیکیورٹی ڈپازٹ',
    'security_deposit_description' => 'قابل واپسی ڈپازٹ، چیک آؤٹ کے بعد تک روکا جاتا ہے اور نقصان نہ ہونے کی صورت میں واپس کر دیا جاتا ہے۔',
    'price_details' => 'قیمت کی تفصیلات',
    'number_of_rooms' => 'کمروں کی تعداد',
    'room' => 'کمرہ',
    'rooms' => 'کمرے',
    'adults' => 'بالغ',
    'adult' => 'بالغ',
    'children' => 'بچے',
    'child' => 'بچہ',
    'email_placeholder' => 'example@gmail.com',
    'phone_number' => 'فون نمبر',
    'phone_note' => 'ہم صرف آپ کی بکنگ کے بارے میں رابطہ کریں گے',
    'book_btn' => 'بک کریں',
    'nights_note' => ':count رات · تاریخاں بدلیں تو قیمت کی تفصیلات اپڈیٹ ہوں گی',
    'nights_note_plural' => ':count راتیں · تاریخاں بدلیں تو قیمت کی تفصیلات اپڈیٹ ہوں گی',
    'err_name_required' => 'نام ضروری ہے',
    'err_name_min' => 'نام کم از کم 2 حروف کا ہونا چاہیے',
    'err_email_required' => 'ای میل ضروری ہے',
    'err_email_invalid' => 'براہ کرم درست ای میل درج کریں',
    'err_phone_required' => 'فون نمبر ضروری ہے',
    'err_phone_invalid' => 'براہ کرم درست فون نمبر درج کریں',
    'toast_success' => 'بکنگ کی درخواست کامیابی سے جمع ہو گئی!',
    'toast_fix_errors' => 'براہ کرم فارم میں غلطیاں درست کریں',
    'payment_method_title' => 'ادائیگی کا طریقہ منتخب کریں',
    'payment_method_subtitle' => 'آپ کس طرح ادائیگی کرنا چاہتے ہیں',
    'payment_cash' => 'نقد ادائیگی',
    'payment_cash_desc' => 'پہنچنے پر میزبان کو براہ راست ادائیگی کریں',
    'payment_online' => 'آن لائن ادائیگی',
    'coming_soon' => 'جلد آ رہا ہے',
    'cancel' => 'منسوخ کریں',
    'confirm_booking' => 'بکنگ کی تصدیق کریں',
];
